...
public class VectorizeJSON implements Vectorize {

    protected static Dictionary dictionary;
    protected FeatureScoring fs;
    protected final Labels labels;

    public VectorizeJSON(Dictionary d, boolean zoning, Labels labels) {
        dictionary = d;
...
//                    }
    ...
    protected void vectorizeLabeled(Corpus corpus, String libsvmFilename, String labelsFile, 
            boolean perLabel, String metaTrainFileName) {
        corpus.reset();
        try (BufferedWriter output = Files.newBufferedWriter(Paths.get(libsvmFilename), Charset.forName("UTF-8"));
            ...
            while ((doc = corpus.nextDocument()) != null) {
                ...
                for (String term : meshTerms) {
                    ...
                    Integer index = labels.getIndex(term);
                    if (index == -1) { //CHANGE in week 5
                        //System.out.println("Label " + line + " not in training corpus");                                
                    } else {
                        docLabels.add(index);
                        cardinality++;
                        TreeSet<Integer> sortedSet;
                        if (labelValues.contains(index)) {
                            sortedSet = labelValues.get(index);
                        } else {
                            sortedSet = new TreeSet<>();
                        }
                        sortedSet.add(document - 1);
                        labelValues.put(index, sortedSet);
                    }
                }
            ...
            if (perLabel) {
                ...
                try (ObjectOutputStream metaTrain = new ObjectOutputStream(new 
        BufferedOutputStream(new FileOutputStream(metaTrainFileName)))) {
